        ...
        <form action="{{ route('admin.biaya.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="keterangan" id="keterangan" rows="3"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-green-500 focus:border-green-500 {{ $errors->has('keterangan') ? 'border-red-500' : 'border-gray-300' }}">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
