    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('admin.siswa.edit', $siswa->id) }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg inline-flex items-center space-x-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                    <span>Edit</span>
                </a>
                <a href="{{ route('admin.siswa.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg inline-flex items-center space-x-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full {{ $siswa->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ \Carbon\Carbon::createFromDate($tagihan->tahun, $tagihan->bulan, 1)->locale('id')->translatedFormat('F') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $tagihan->tahun }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rp {{ number_format($tagihan->jumlah, 0, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rp {{ number_format($tagihan->jumlah_bayar, 0, ',', '.') }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('admin.tagihan.show', $tagihan->id) }}" class="text-green-600 hover:text-green-900">Detail</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <p class="text-xl font-bold text-gray-800">Rp {{ number_format($siswa->tagihan->sum('jumlah'), 0, ',', '.') }}</p>
            </div>
            <div class="bg-green-50 rounded-lg p-4">
                <p class="text-sm text-gray-500 mb-1">Total Dibayar</p>
                <p class="text-xl font-bold text-green-800">Rp {{ number_format($siswa->tagihan->sum('jumlah_bayar'), 0, ',', '.') }}</p>
            </div>
            <div class="bg-red-50 rounded-lg p-4">
                <p class="text-sm text-gray-500 mb-1">Total Tunggakan</p>
                <p class="text-xl font-bold text-red-800">Rp {{ number_format($siswa->tagihan->sum('jumlah') - $siswa->tagihan->sum('jumlah_bayar'), 0, ',', '.') }}</p>
            </div>
        </div>
        @else
